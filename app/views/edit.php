...
<head>
    ...
    <title><?= $isEdit ? 'Edit' : 'Add' ?> Item - Trade Coin</title>
...
<body class="bg-white p-6 md:p-10 font-sans text-gray-800">
    ...
    <div class="max-w-5xl mx-auto">
        <header class="flex justify-between items-start mb-8">
            <a href="/onsale" class="font-bold text-lg flex items-center hover:opacity-70 transition">
            ...
            <div class="bg-white/40 backdrop-blur-sm border border-gray-400/40 p-8 md:p-12 rounded-[2rem] flex flex-col md:flex-row items-center gap-10">
                ...
                <form action="<?= $isEdit ? '/onsale/update/' . $item['id'] : '/onsale/store' ?>" method="POST" enctype="multipart/form-data" class="flex flex-col md:flex-row items-center w-full gap-10">
                    ...
                    <div class="relative group">
                        <img id="imagePreview" src="<?= $isEdit && !empty($item['gambar']) ? htmlspecialchars($item['gambar']) : '/public/foto/scara.png' ?>" alt="Item Preview" class="w-48 h-48 md:w-64 md:h-64 rounded-3xl object-cover shadow-md border-2 border-white/50">
                        <label for="imageInput" class="absolute inset-0 bg-black/10 rounded-3xl opacity-0 group-hover:opacity-100 transition flex items-center justify-center cursor-pointer">
                    ...
                    <div class="flex-1 w-full space-y-4">
                        <input type="text" name="name" placeholder="Name" required
                            value="<?= $isEdit ? htmlspecialchars($item['nama_produk'] ?? '') : '' ?>"
                        ...
                        <textarea name="description" placeholder="Description" rows="3" required
                            class="w-full bg-white/60 border border-gray-400 rounded-xl px-4 py-2 focus:outline-none focus:bg-white transition placeholder-gray-500"><?= $isEdit ? htmlspecialchars($item['deskripsi'] ?? '') : '' ?></textarea>
                        
                        <input type="number" name="price" placeholder="Price (optional)" 
                            value="<?= $isEdit ? htmlspecialchars($item['harga'] ?? '') : '' ?>"
                        ...
                        <input type="text" name="category" placeholder="Category (optional)" 
                            value="<?= $isEdit ? htmlspecialchars($item['category'] ?? '') : '' ?>"
                            class="w-full bg-white/60 border border-gray-400 rounded-xl px-4 py-2 focus:outline-none focus:bg-white transition placeholder-gray-500">
                        
                        <input type="text" name="material" placeholder="Material (optional)" 
                            value="<?= $isEdit ? htmlspecialchars($item['material'] ?? '') : '' ?>"
                            class="w-full bg-white/60 border border-gray-400 rounded-xl px-4 py-2 focus:outline-none focus:bg-white transition placeholder-gray-500">
